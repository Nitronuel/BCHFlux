...
const BridgePage: React.FC = () => {
    const { isConnected, balance, accountMode } = useWalletStore();
    ...
    const fetchQuote = async (isSilent = false) => {
        ...
        try {
            ...
            console.error(err);
            setQuote(null);
            setErrorMsg(err.response?.data?.message || 'Route unavailable. Bridge is currently down for this asset.');
        } finally {
    ...
    const handleSwap = async () => {
        ...
        setLoading(true);
        try {
            // 1. Fetch the REAL transaction payload from the backend
            // In demo mode, we automatically generate an address for the target chain
            const autoGeneratedTargetAddress = autoGenerateAddress(toChain);
            ...
            const txResponse = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/bridge/tx/create`, {
                requestId: quote.quoteId,
                userWalletAddress: autoGeneratedTargetAddress,
                isDemo
            });

            const txPayload = txResponse.data;

            if (!txPayload || !txPayload.memo) {
                throw new Error("Failed to generate cross-chain routing payload.");
            }

            // 2. Extract real Memo and Address
            const finalMemo = txPayload.memo;
            const finalToAddress = txPayload.to; // Where the BCH needs to be sent (e.g., THORChain router)
            const finalValue = quote.amountIn; // The amount of BCH to send

            // 3. Sign & Send Real Transaction via mainnet-js
            const tx = await WalletService.sendBch(finalToAddress, { bch: finalValue }, finalMemo, isDemo);
            setTxId(tx);

            // 4. Update History with real TX ID
            addTransaction({
                txId: tx,
                fromToken: 'BCH',
                toToken,
            ...
            <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center p-4 bg-background relative">
                <div className="w-full max-w-md space-y-4">
                    ...
                    <div className="bg-surface border border-border rounded-2xl p-4 shadow-xl">
                        ...
                        {/* Error Message */}
                        {errorMsg && (
                            <div className="mt-4 p-3 bg-red-500/10 border border-red-500/20 text-red-500 rounded-lg text-sm flex items-center gap-2 animate-in fade-in">
